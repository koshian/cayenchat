...
- **System secure storage** (default): macOS Keychain, Windows Credential Manager, or on Linux and other Unix desktops the freedesktop Secret Service (GNOME Keyring, KWallet or another provider), through the [`keyring`](https://crates.io/crates/keyring) crate. Entries use the service name `CayenChat` and names such as `connection/<server-id>/sasl-password` or `uploader/gyazo/default/credential`, built from internal IDs rather than hostnames or nicknames.
...
**Gyazo** is currently supported. Gyazo's OAuth sign-in requires a client secret that an open-source desktop application cannot keep private, so CayenChat connects with a personal access token instead: open the [Gyazo developer dashboard](https://gyazo.com/oauth/applications), create an application, open it and generate an **access token** (CayenChat does not use the application's callback URL), then choose **Connect Account** in CayenChat and paste it. The token is stored in the credential storage; **Reconnect Account** replaces it and **Disconnect Account** deletes it. Gyazo tokens do not expire but stop working if you delete the application. No client ID or secret is shipped with CayenChat.

To send an image, paste an image (`Cmd+V` / `Ctrl+V`) into a draft whose clipboard holds an image and no text, or drop one image file (PNG, JPEG, GIF, WebP, BMP or TIFF, up to 32 MB) onto the draft row. Clipboard text always pastes as text. CayenChat then asks before anything leaves your computer, naming the provider. After **Upload**, the draft row shows progress; the link is inserted at the draft's cursor only after a successful upload, and nothing is sent until you press Enter. **Cancel** in the dialog uploads nothing and leaves the draft unchanged; cancelling during the upload stops waiting, but the image may already have reached the provider. Failures keep the draft as it was. If no provider is configured, or Gyazo rejects the token, the dialog offers to open the Image Upload settings.

Uploaded images are kept by the hosting service, not by IRC. Gyazo uploads made this way use Gyazo's default access policy, under which anyone who has the link can view the image. People outside the channel can view it if they obtain the link; channel membership provides no access control. Links are not shown inline in the chat log yet.
