...
impl WindowsWindowInner {
    ...
    fn handle_syskeydown_msg(&self, handle: HWND, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
        ...
        let Some(input) = handle_key_event(handle, wparam, lparam, &mut lock, |keystroke| {
            ...
            drop(lock);
            translate_message(handle, WM_SYSKEYDOWN, wparam, lparam);
            return None;
        };
        let Some(mut func) = lock.callbacks.input.take() else {
            drop(lock);
    ...
    fn handle_keydown_msg(&self, handle: HWND, wparam: WPARAM, lparam: LPARAM) -> Option<isize> {
        ...
        let Some(input) = handle_key_event(handle, wparam, lparam, &mut lock, |keystroke| {
            ...
            drop(lock);
            // IME mode keys may have no GPUI keystroke, but still need Win32 translation.
            translate_message(handle, WM_KEYDOWN, wparam, lparam);
            return None;
        };
        drop(lock);
...
#[inline]
fn translate_message(handle: HWND, message: u32, wparam: WPARAM, lparam: LPARAM) {
    let msg = MSG {
        hwnd: handle,
        message,
        wParam: wparam,
